...
namespace Java.Interop
{
	partial class JniEnvironment
	{
		...
		{
#if !NETSTANDARD2_0
			public static unsafe JniFieldInfo GetFieldID (JniObjectReference type, ReadOnlySpan<char> name, ReadOnlySpan<char> signature)
			{
				if (!type.IsValid)
				...
				fixed (char* ptr = signature) {
					var tmp = NativeMethods.java_interop_jnienv_get_field_id (JniEnvironment.EnvironmentPointer, out var thrown, type.Handle, name.ToString (), ptr);
