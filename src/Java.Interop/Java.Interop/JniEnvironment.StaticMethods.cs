...
namespace Java.Interop
{
	partial class JniEnvironment
	{
		...
		{
#if !NETSTANDARD2_0
			internal static unsafe JniMethodInfo GetStaticMethodID (JniObjectReference type, ReadOnlySpan<char> name, ReadOnlySpan<char> signature)
			{
				if (!type.IsValid)
				...
				fixed (char* sig = &MemoryMarshal.GetReference (signature)) {
					...
					var tmp = NativeMethods.java_interop_jnienv_get_static_method_id (JniEnvironment.EnvironmentPointer, out var thrown, type.Handle, name.ToString (), sig);
